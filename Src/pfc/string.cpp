...
void string_a::s_SetWindowText(HWND w) { SetWindowTextA(w, *this); }
#else
// Win32 window-text accessors; no native equivalent until the Qt6 GUI port
// (Phase 4). Defined as no-ops so the static lib links on Linux.
void string_a::s_GetWindowText(HWND) { reset(); }

void string_a::s_SetWindowText(HWND) {}
#endif
...
{
        RegSetValueExA(hk,name,0,REG_SZ,(const BYTE*)(const
char*)*this,(length()+1));
}
*/