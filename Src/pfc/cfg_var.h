...
protected:
  cfg_var(const char *name) : var_name(name) {
    next = list;
    list = this;
  };
...
public:
#ifdef _WIN32
  // wchar_t is 16-bit on MSVC (== WCHAR); on Linux it is 32-bit and does not
  // match string_utf8(const WCHAR*). Win32-only until a wchar_t overload
  // exists.
  cfg_int(const wchar_t *name, int v) : cfg_var(string_utf8(name)) {
    val = def = v;
  }
#endif
...
public:
  cfg_string_w(const char *name, const WCHAR *v)
      : cfg_var(name), val(v), def(v) {}
  cfg_string_w(const char *name, const char *v)
      : cfg_var(name), val(string_w(v)), def(string_w(v)) {}
...
template <class T> class cfg_struct_t : private cfg_var {
private:
  T val, def;

  virtual void read(HKEY hk) { reg_read_struct(hk, &val, sizeof(T)); }
...
public:
  ...
  cfg_struct_t(const char *name, int filler) : cfg_var(name) {
    memset(&val, filler, sizeof(T));
    memset(&def, filler, sizeof(T));
  }
