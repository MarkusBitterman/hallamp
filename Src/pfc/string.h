...
// Win32 types (UINT/WCHAR/HWND/HKEY) came transitively via <windows.h> on
// Windows; on Linux they come from the Wasabi platform layer. Make the header
// self-contained so it doesn't depend on includer ordering.
#ifdef _WIN32
#include <windows.h>
#else
...
// C++ wrappers, NOT <string.h>/<stdlib.h>: this file is itself named string.h,
// so a bare <string.h> would self-resolve under -I of this directory.
...
private:
  ...
  static int mylen(const myChar *p) {
    int r = 0;
    while (p[r])
      r++;
    return r;
...
public:
  ...
  void set_string(const myChar *s) {
    reset();
    add_string(s);
  }
...
template <class myChar> class string_buffer {
private:
  string_base<myChar> *parent;
  myChar *data;

public:
  string_buffer(string_base<myChar> &s, UINT siz) {
    parent = &s;
    data = s.buffer_get(siz);
  }
...
public:
  ...
  const char *operator=(const WCHAR *s) {
    set_string_w(s);
    return get_ptr();
  }
  const char *operator+=(const WCHAR *s) {
  ...
  const char *operator=(string_a &s) {
    set_string(s);
    return get_ptr();
  }
  const char *operator+=(string_a &s) {
...
#ifdef _WIN32
  inline void s_GetDlgItemText(HWND w, int id) {
    s_GetWindowText(GetDlgItem(w, id));
  }
  inline void s_SetDlgItemText(HWND w, int id) {
    s_SetWindowText(GetDlgItem(w, id));
  }
#endif
...
template <class myChar> class string_file_title : public string_base<myChar> {
public:
...
template <class myChar> class string_extension : public string_base<myChar> {
public:
