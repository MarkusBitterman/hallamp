...
public:
  ...
  enum {
    // pass this flag to AddHandle or RunFunction indicate that your thread
    // function might run for a long time
...
public:
  ...
  /* adds a waitable handle to the thread pool.  when the event is signalled,
  your function ptr will get called user_data and id values get passed to your
  function. your function should return 1 to indicate that it can be removed
  flags, see api_threadpool	*/
  int AddHandle(ThreadID *threadid, HANDLE handle,
                api_threadpool::ThreadPoolFunc func, void *user_data,
                intptr_t id, int flags);
  void RemoveHandle(ThreadID *threadid, HANDLE handle);
  int RunFunction(ThreadID *threadid, api_threadpool::ThreadPoolFunc func,
                  void *user_data, intptr_t id, int flags);

  size_t GetNumberOfThreads(); // total number of threads in the threadpool
  size_t
  GetNumberOfActiveThreads(); // number of threads that are currently being used
                              // (inside user function but not necessarily busy)
...
inline int api_threadpool::AddHandle(ThreadID *threadid, HANDLE handle,
...
inline int api_threadpool::RunFunction(ThreadID *threadid,
                                       api_threadpool::ThreadPoolFunc func,
                                       void *user_data, intptr_t id,
                                       int flags) {
...
static const GUID ThreadPoolGUID = {
    ...
    0x11d8,
    0x4ac6,
    {0xa3, 0xe6, 0x21, 0x6d, 0xf5, 0x25, 0x21, 0x7}};
