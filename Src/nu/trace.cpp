#include "./trace.h"
#include <stdlib.h> // realloc/free (was pulled transitively via <windows.h> on Win32)
#include <strsafe.h>
...
void DebugPrintfW(LPCWSTR format, ...) {
  ...
  } while (STRSAFE_E_INSUFFICIENT_BUFFER ==
           StringCchVPrintfExW(pstr, allocated, NULL, &remaining,
  ...
  do {
    ...
    if (attempt) {
      ...
      pstr = (char *)buffer;
           ...
           StringCchVPrintfExA(pstr, allocated, NULL, &remaining,
                               STRSAFE_NULL_ON_FAILURE, format, argList));
