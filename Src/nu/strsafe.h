...
#else
// Linux port: the wide vprintf workers below were left #ifdef _WIN32 ("TODO:
// benski> port to BSD") because they call MSVC's _vsnwprintf. glibc's vswprintf
// has the same signature and the same -1-on-truncation contract the workers
// already rely on, so alias it and un-gate the wide path. NOTE: this operates
// on the platform-native 32-bit wchar_t (what trace.cpp and libc use), NOT the
// codebase's 16-bit WCHAR. Reconciling the WCHAR* callers is a separate bite.
...
#define __stdcall
// STRSAFE_LIB_IMPL (strsafe.c) emits __declspec(dllexport); neutralize it here
// so strsafe.h stays self-contained on Linux (it doesn't pull the Wasabi
// platform shim, which also defines this). Identical redefinition is harmless
// if both meet.
#ifndef __declspec
#define __declspec(a)
#endif
...
#if 1 // ported to Linux: wide path uses native-wchar_t vswprintf (was #ifdef
      // _WIN32)
STRSAFEAPI StringCchVPrintfW(wchar_t *pszDest, size_t cchDest,
                             const wchar_t *pszFormat, va_list argList) {
  HRESULT hr;
...
#if 1 // ported to Linux: _vsnwprintf is aliased to vswprintf above (was #ifdef
      // _WIN32)
STRSAFEAPI StringVPrintfWorkerW(wchar_t *pszDest, size_t cchDest,
                                const wchar_t *pszFormat, va_list argList) {
  HRESULT hr = S_OK;
