...
void OutputDebugStringW(const wchar_t *s) {
  // native wchar_t -> multibyte, then reuse the narrow sink. nu/trace.cpp's
  // wide path is native wchar_t (matching the strsafe.h wide port), not 16-bit
  // WCHAR.
  if (!s)
    return;
...
Window Linux::RootWin() { return RootWindow(getDisplay(), getScreenNum()); }
  ...
  case MotionNotify: {
    ...
    do {
      // Spin...
        ...
        XCheckTypedWindowEvent(Linux::getDisplay(), m->hwnd, MotionNotify, e));
    ...
    if (message == dnd_enter) {
      if (e->xclient.data.l[1] & 1) {
        ...
        XGetWindowProperty(Linux::getDisplay(), e->xclient.data.l[0],
                           dnd_typelist, 0, 65536, True, XA_ATOM, &actual,
      ...
      XSendEvent(Linux::getDisplay(), e->xclient.data.l[0], False, NoEventMask,
                 (XEvent *)&cme);
      ...
      if (supported != None) {
        ...
        XGetWindowProperty(Linux::getDisplay(), cme.window, dnd_wa3drop, 0,
                           65536, True, supported, &actual, &format, &nitems,
                           &bytes, &data);
...
HWND GetActiveWindow() { return activeWindow; }
...
void PostQuitMessage(int i) { PostMessage(None, WM_QUIT, i, 0); }
...
int _GetMessage(MSG *m, HWND, UINT, UINT, int block = 1) {
  ...
  if (qid == -1) {
    qid = WASABI_API_LINUX->linux_getIPCId();
  }
...
int PeekMessage(MSG *m, HWND w, UINT f, UINT l, UINT remove) {
  if (remove == PM_NOREMOVE)
    return _GetMessage(m, w, f, l, 0);
  else
    _GetMessage(m, w, f, l, 1);
}
...
void ExitProcess(int ret) { exit(ret); }
...
int SubtractRect(RECT *out, RECT *in1, RECT *in2) {
  ...
  if (in1->left >= in2->left && in1->right <= in2->right) {
    ...
    if (in1->top >= in2->top && in2->bottom >= in2->top &&
        in2->bottom <= in2->bottom) {
      out->top = in1->bottom;
      out->bottom = in2->bottom;

      ret = 1;
    } else if (in1->top <= in2->top && in1->bottom >= in2->top &&
    ...
    out->top = in1->top;
    out->bottom = in1->bottom;

    if (in1->left >= in2->left && in2->right >= in2->left &&
        in2->right <= in2->right) {
      out->left = in1->right;
      out->right = in2->right;

      ret = 1;
    } else if (in1->left <= in2->left && in1->right >= in2->left &&
               in1->right <= in2->right) {
      out->left = in1->left;
      out->right = in2->left;
...
HWND WindowFromPoint(POINT p) {
  ...
  XTranslateCoordinates(Linux::getDisplay(), Linux::RootWin(), Linux::RootWin(),
                        p.x, p.y, &x, &y, &child);
  return child;
}
#endif // wnd

void CopyFile(const char *f1, const char *f2, BOOL b) { COPYFILE(f1, f2); }
