...
int wa::Components::WAC_Network_AsyncDNS::resolvenow(const char *hostname,
                                                     unsigned short port,
                                                     addrinfo **addr,
                                                     int sockettype) {
  addrinfo hints;
...
#ifdef _WIN32
unsigned long
    WINAPI wa::Components::WAC_Network_AsyncDNS::_threadfunc(LPVOID _d)
#else
unsigned int wa::Components::WAC_Network_AsyncDNS::_threadfunc(void *_d)
#endif
{
  int nowinsock = JNL::open_socketlib();
  wa::Components::WAC_Network_AsyncDNS *_this =
      (wa::Components::WAC_Network_AsyncDNS *)_d;
  int x;
    ...
    if (_this->m_cache[x].last_used && !_this->m_cache[x].resolved) {
      if (!nowinsock) {
        if (_this->m_cache[x].mode == 0) {
          addrinfo *res = 0;
          if (resolvenow(_this->m_cache[x].hostname, _this->m_cache[x].port,
                         &res, _this->m_cache[x].sockettype) == 0) {
...
int wa::Components::WAC_Network_AsyncDNS::resolve(const char *hostname,
                                                  unsigned short port,
                                                  addrinfo **addr,
                                                  int sockettype) {
  ...
  for (x = 0; x < m_cache_size; x++) {
    if (!strcasecmp(m_cache[x].hostname, hostname) && port == m_cache[x].port &&
        m_cache[x].mode == 0 && m_cache[x].sockettype == sockettype) {
  ...
  for (x = 0; x < m_cache_size; x++) {
    ...
    if ((oi == -1 || m_cache[x].last_used < m_cache[oi].last_used) &&
        m_cache[x].resolved) {
      oi = x;
    }
  }
...
void wa::Components::WAC_Network_AsyncDNS::makesurethreadisrunning(void) {
  if (m_thread_kill) {
    ...
    if (m_thread) {
      void *p;
      pthread_join(m_thread, &p);
      m_thread = 0; // handle is invalid after join; don't join a stale id
    }
    m_thread_kill = 0;
    if (pthread_create(&m_thread, NULL, (void *(*)(void *))_threadfunc,
                       (void *)this) != 0) {
#endif
      m_thread = 0; // create failed: keep the guard honest (no stale handle)
      m_thread_kill = 1;
    }
  }
}
