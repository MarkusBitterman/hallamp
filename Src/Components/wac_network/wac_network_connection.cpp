...
WAC_Network_Connection::WAC_Network_Connection() { init(); }

WAC_Network_Connection::WAC_Network_Connection(api_dns *dns, size_t sendbufsize,
                                               size_t recvbufsize) {
  init();
...
WAC_Network_Connection::~WAC_Network_Connection() {
  ...
  */
  ...
  if (!saddr)      // free it if it was passed to us (by JNL_Listen, presumably)
    free(address); // TODO: change this if we ever do round-robin DNS connecting
                   // or in any way change how we handle 'address'
...
void WAC_Network_Connection::open(api_dns *dns, size_t sendbufsize,
                                  size_t recvbufsize) {
...
void WAC_Network_Connection::connect(SOCKET s, sockaddr *addr,
                                     socklen_t length) {
  close(1);
  m_socket = s;
...
*/
/* Virtual */
ssize_t WAC_Network_Connection::socket_recv(char *buf, size_t len,
                                            int options) {
...
*/
/* Virtual */
ssize_t WAC_Network_Connection::socket_send(const char *buf, size_t len,
                                            int options) {
...
void WAC_Network_Connection::run(size_t max_send_bytes, size_t max_recv_bytes,
                                 size_t *bytes_sent, size_t *bytes_rcvd) {
  ...
  size_t bytes_allowed_to_send =
      (max_send_bytes == (size_t)-1) ? send_buffer.size() : max_send_bytes;
  size_t bytes_allowed_to_recv =
      (max_recv_bytes == (size_t)-1) ? recv_buffer.avail() : max_recv_bytes;
  ...
  case STATE_RESOLVED:
    ...
    getsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, (char *)&socket_buffer_size,
    ...
    if (socket_buffer_size < send_buffer_size)
      setsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, (char *)&send_buffer_size,
                 sizeof(send_buffer_size));
    getsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, (char *)&socket_buffer_size,
               &socket_buffer_size_len);

    getsockopt(m_socket, SOL_SOCKET, SO_RCVBUF, (char *)&socket_buffer_size,
               &socket_buffer_size_len);
    recv_buffer_size = (int)recv_buffer.avail();
    if (recv_buffer_size > 65536)
      recv_buffer_size = 65536;
    if (socket_buffer_size < recv_buffer_size)
      setsockopt(m_socket, SOL_SOCKET, SO_RCVBUF, (char *)&recv_buffer_size,
...
void WAC_Network_Connection::on_socket_connected(void) { return; }
...
case API_CONNECTION_CONNECT:
connect(*(char **)(params[0]), *(int *)(params[1]));
return 1;
