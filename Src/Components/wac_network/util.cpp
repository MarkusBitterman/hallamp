...
static HCRYPTPROV GetKeySet() {
  ...
  {
    return hCryptProv;
  } else if (CryptAcquireContext(&hCryptProv, UserName, NULL, PROV_RSA_FULL,
                                 CRYPT_NEWKEYSET)) {
    return hCryptProv;
  } else
    return 0;
...
int JNL::parse_url(const char *url, char **prot, char **host,
                   unsigned short *port, char **req, char **lp) {
  free(*prot);
  *prot = 0;
  free(*host);
  *host = 0;
