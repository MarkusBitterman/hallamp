...
** File: wac_network_http_server.h - JNL interface for doing HTTP GET/POST
* serving.
...
public:
  ...
  int run(); // returns: < 0 on error, 0 on request not read yet, 1 if reading
             // headers, 2 if reply not sent, 3 if reply sent, sending data. 4
  ...
  const char *
  get_request_parm(const char *parmname); // parameter portion (after ?)
  const char *getallheaders() {
    return recvheaders.GetAllHeaders();
  ...
  void set_reply_string(
      const char *reply_string); // should be HTTP/1.1 OK or the like
  void add_reply_header(const char *header); // i.e. "content-size: 12345"

  void send_reply() {
    m_reply_ready = 1;
  } // send reply, state will advance to 3.

  ////////// sending data ///////////////
  int bytes_inqueue() {
    if (m_state == 3 || m_state == -1 || m_state == 4)
      return (int)m_con->send_bytes_in_queue();
    else
      return 0;
  }
  int bytes_cansend() {
    if (m_state == 3)
      return (int)m_con->send_bytes_available();
    else
      return 0;
  }
...
protected:
  ...
  char *m_recv_request; // either double-null terminated, or may contain
                        // parameters after first null.
  char *m_method;
