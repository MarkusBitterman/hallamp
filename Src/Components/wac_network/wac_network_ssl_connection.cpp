...
JNL_SSL_Connection::JNL_SSL_Connection()
    : forceConnect(false), m_ssl(0), m_bsslinit(false), m_bcontextowned(true) {
  m_bsslinit = true;
}

JNL_SSL_Connection::JNL_SSL_Connection(SSL *pssl, api_dns *dns,
                                       size_t sendbufsize, size_t recvbufsize)
    : WAC_Network_Connection(dns, sendbufsize, recvbufsize),
      forceConnect(false) {
  m_ssl = pssl;
  m_bsslinit = false;
...
int JNL_SSL_Connection::socket_connect() {
  ...
  if (retval != 0) {
    if (ERRNO != EINPROGRESS) {
      return retval; // benski> if the underlying socket hasn't connected yet,
                     // then we can't start the SSL connection
                     /*
  ...
  /*
  ** Set the SNI hostname. This 2007-era code predates universal SNI; modern
  ** TLS servers (CDNs, virtual hosts) abort the handshake with a fatal alert
  ** if the ClientHello carries no server_name. m_host is the connect target,
  ** populated by WAC_Network_Connection::connect(hostname, port).
  */
  if (m_host[0])
...
void JNL_SSL_Connection::run(size_t max_send_bytes, size_t max_recv_bytes,
  ...
  /**
   ** benski - march 2, 2006
   **if the underlying socket didn't connected yet, we need to try the SSL
   * connection again
   */
  if (forceConnect) {
  ...
  WAC_Network_Connection::run(max_send_bytes, max_recv_bytes, bytes_sent,
                              bytes_rcvd);
}
...
bool JNL_SSL_Connection::init_ssl_connection() {
  ...
  if (retval < 0) {
    ...
    case SSL_ERROR_WANT_CONNECT:
      ...
      break;
      // fall through
    default: // TODO: benski> MOST other errors are OK, (especially "want read"
             // and "want write", but we need to think through all the scenarios
             // here
      forceConnect = false;
    }
  } else if (retval) {
...
void JNL_SSL_Connection::on_socket_connected(void) { init_ssl_connection(); }
...
ssize_t JNL_SSL_Connection::socket_send(const char *buf, size_t len,
                                        int options) {
