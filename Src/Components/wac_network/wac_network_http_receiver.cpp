...
wa::Components::WAC_Network_HTTPGet::WAC_Network_HTTPGet(api_dns *p_dns,
                                                         size_t p_recvbufsize,
                                                         const char *p_proxy) {
  JNL::open_socketlib();
...
void wa::Components::WAC_Network_HTTPGet::open(api_dns *p_dns,
                                               size_t p_recvbufsize,
                                               const char *p_proxy) {
  ...
  if (p_proxy) {
    ...
    if (p) {
      char *r = NULL;
      do_parse_url(p, &m_http_proxyhost, &m_http_proxyport, &r,
                   &m_http_proxylpinfo);
...
void wa::Components::WAC_Network_HTTPGet::deinit(bool p_full) {
  if (!persistent || p_full ||
      (m_con && m_con->get_state() == WAC_Network_Connection::STATE_ERROR)) {
    delete m_con;
    m_con = NULL;
  }
...
void wa::Components::WAC_Network_HTTPGet::set_sendbufsize(
    size_t p_sendbufsize) {
  m_sendbufsize = p_sendbufsize;
}

int wa::Components::WAC_Network_HTTPGet::set_recv_buffer_size(
    size_t p_new_buffer_size) {
  if (m_con) {
    int ret = m_con->set_recv_buffer_size(p_new_buffer_size);
    if (ret == NErr_NoAction) // this will get returned if new_buffer_size is
...
void wa::Components::WAC_Network_HTTPGet::addheadervalue(const char *header,
                                                         const char *value) {
...
void wa::Components::WAC_Network_HTTPGet::do_encode_mimestr(char *in,
                                                            char *out) {
  char alphabet[] =
...
void wa::Components::WAC_Network_HTTPGet::connect(const char *url, int ver,
                                                  const char *requestmethod) {
  deinit(false);
  m_http_url = _strdup(url);
  do_parse_url(m_http_url, &m_http_host, &m_http_port, &m_http_request,
               &m_http_lpinfo);
  ...
  if (!m_http_proxyhost || !m_http_proxyhost[0])
    sendbufferlen += strlen(requestmethod) + 1 /* GET */ +
                     strlen(m_http_request) + 9 /* HTTP/1.0 */ + 2;
  else {
    sendbufferlen += strlen(requestmethod) + 1 /* GET */ + strlen(m_http_url) +
                     9 /* HTTP/1.0 */ + 2;
  ...
  if (!m_http_proxyhost || !m_http_proxyhost[0]) {
    ...
    if (strncasecmp(m_http_url, "uvox://", 7) == 0) {
      myp = m_http_url + 7;
      StringCchPrintfExA(str, strLen, &str, &strLen, 0,
                         "%s http: //%s HTTP/1.%d\r\n", requestmethod, myp,
                         ver % 10);
  ...
  if (m_http_proxylpinfo && m_http_proxylpinfo[0]) {
    StringCchCatExA(str, strLen, "Proxy-Authorization: Basic ", &str, &strLen,
                    0);
    ...
    */
    if (!_strnicmp(m_http_url, "https:", strlen("https:"))) {
      ...
      if (m_sendbufsize == 0 && !strcmp(requestmethod, "POST"))
        send_buffer_size += 8192; // some extra room for posting data if it
...
void wa::Components::WAC_Network_HTTPGet::do_parse_url(const char *url,
                                                       char **host,
                                                       unsigned short *port,
                                                       char **req, char **lp) {
  char *l_port = 0;
    ...
    if (m_con->recv_lines_available() > 0) {
      ...
      if (code >= 200 && code <= 206)
        ...
        m_http_state = 1; // redirect city
                 ...
                 100) // in case of HTTP 100 Continue code, we'll keep looping
      {
  ...
  if (m_http_state == 2) {
    ...
    while (m_con->recv_lines_available() > 0) {
      ...
      if (!m_recvheaders) {
        ...
        if (m_recvheaders_size == 0 ||
            m_recvheaders_size == (size_t)-1) // check for overflow
        {
          m_http_state = -1;
          return HTTPRECEIVER_RUN_ERROR;
        }
        ...
        if (m_recvheaders) {
          strcpy(m_recvheaders,
                 buf); // safe because we malloc'd specifically above
        ...
        strcpy(n + oldsize,
               buf); // safe because we malloc specifially for the size
...
int wa::Components::WAC_Network_HTTPGet::
    get_status() // returns 0 if connecting, 1 if reading headers, 2 if reading
                 // content, -1 if error.
{
  if (m_http_state < 0)
...
int wa::Components::WAC_Network_HTTPGet::
    getreplycode() // returns 0 if none yet, otherwise returns http reply code.
{
  if (!m_reply)
    return 0;
    ...
    if (zlibStream) {
      ...
      if (size) {
        ...
        if (zlib_err == Z_OK || zlib_err == Z_STREAM_END) {
          m_con->recv_bytes(
              0, size - zlibStream->avail_in); // since we only peeked above
...
size_t wa::Components::WAC_Network_HTTPGet::peek_bytes(char *buf, size_t len) {
  if (m_con && m_http_state == 3) {
    if (zlibStream)
      return 0; // TODO: benski> how are we going to do peek_bytes, since the
                // inflater saves state?
    else
